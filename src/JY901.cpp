...
bool readJY901_angles(uint8_t addr, JY901S_AngleData &out) {// 解析角度寄存器 0x3D..0x42（6 字节，小端）
  ...
  // out.roll = (float)out.roll / scale * 180;
  // out.pitch = (float)out.pitch / scale * 180;
  // out.yaw = (float)out.yaw / scale * 180;
  return true;
}
...
bool readJY901_accel(uint8_t addr, JY901S_AccelData &out) {// 解析加速度寄存器（尝试寄存器基址 0x34..0x39）
  ...
  out.ax = (float)out.ax / scale;
  out.ay = (float)out.ay / scale;
  out.az = (float)out.az / scale;
  return true;
}
