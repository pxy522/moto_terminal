...
// ==================== IMU数据 ====================
extern float jy901_vx;                   // X方向线速度(m/s)
extern float jy901_vy;                   // Y方向线速度(m/s)
extern float jy901_vz;                   // Z方向线速度(m/s)
JY901S_AngleData ang;                    // 角度数据结构体
JY901S_AccelData acc;                    // 加速度数据结构体
...
void loop() {
    ...
    if (jy901_timer_flag) {
        ...
        if (tcaSelect(1)) {
            if (i2cAddressExists(JY901_ADDR)) {
                // 读取原始数据
                readJY901_raw(JY901_ADDR);
                
                // 解析角度数据
                if (readJY901_angles(JY901_ADDR, ang)) {
                    float roll = (float)ang.roll / 100.0f;
                    float pitch = (float)ang.pitch / 100.0f;
                    float yaw = (float)ang.yaw / 100.0f;
                    // 可选：发送角度数据到BLE
                    // BLE_sendf("Parsed Angles->Roll:%.2f, Pitch:%.2f, Yaw:%.2f\r\n", roll, pitch, yaw);
                }
                
                // 解析加速度数据并进行数据处理
                if (readJY901_accel(JY901_ADDR, acc)) {
                    DP_jy901(acc.ax_g, acc.ay_g, acc.az_g);
                    // 可选：发送加速度数据到BLE
                    // BLE_sendf("Parsed Accel->ax:%.3fg, ay:%.3fg, az:%.3fg\r\n", acc.ax_g, acc.ay_g, acc.az_g);
                }
            }
            I2C_Clear();  // 恢复I2C总线
        }
    ...
    if (five_sec_flag) {
        ...
        // IMU数据
        Serial2.printf("main.x0.val=%05d\xff\xff\xff", (int)(acc.ax_g*1000));
        Serial2.printf("main.x1.val=%05d\xff\xff\xff", (int)(acc.ay_g*1000));
        Serial2.printf("main.x2.val=%04d\xff\xff\xff", (int)(jy901_vx*100));
        Serial2.printf("main.x3.val=%04d\xff\xff\xff", (int)(jy901_vy*100));
